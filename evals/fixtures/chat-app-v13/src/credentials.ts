...
// A static token (VITE_USER_TOKEN) wins when set - the eval fixture mints one per app.
export const tokenProvider = async (): Promise<string> => {
  if (import.meta.env.VITE_USER_TOKEN) return import.meta.env.VITE_USER_TOKEN;
