...
| **Metadata placement** — timestamp / receipt / name moved relative to the bubble (beside it, name above, inside as an overlay) | Core **`messageContent`** slot — a **core** slot, registered on `StreamComponentBuilders(messageContent: …)` directly, **not** under `extensions:`. The override receives the row's parts **already built** — `props.header`, `props.child` (the fully-composed bubble: text, attachments, reactions), `props.footer` (timestamp + receipt) — so the job is re-arranging prebuilt widgets: beside-the-bubble = `Row[child, footer]`, name-above = `Column[header, …]`, inside = `Stack` the footer over `child`. It renders **inside the row's gesture layer**, so the long-press actions modal, reaction picker, and grouping all survive (device-verified). Branch per side via `StreamMessageLayout.messageAlignmentOf(context)` / `crossAxisAlignmentOf(context)`; write it as an inline lambda with the props type **inferred** (`StreamMessageContentProps` isn't nameable in app code). Do **not** reach for `messageBuilder` / `messageItem` for this — the full-row rebuild loses the private long-press/reaction internals (Step 2.5) that this slot keeps for free. | — |
