...
- [ ] **Vertical alignment / centering of each button against the field.** Both the composer **bar** row and the input-**pill** row are `Row(crossAxisAlignment: end)`, so a custom leading / trailing / inputLeading / inputTrailing widget **bottom-anchors** — a bare icon box then sits *low*, its center below the field's text center (the classic "`+` / mic rides low" miss). Fix it the way the SDK's own `Default*` slots do: wrap each side glyph in a box of the **measured field height** + `Center`. Apply it to **every** side slot in one pass — centering the leading `+` but leaving the trailing mic a bare box leaves the two at different heights.
