...
- **Prebuilt components carry an opinionated default look.** The native `Poll` renders large + full-width and *inherits the message bubble background* (scope your own-message colour to `.str-chat__message-text-inner`, not `.str-chat__message-bubble`, or it bleeds onto polls/attachments). The `MessageComposer` placeholder defaults to "Send a message" (`additionalTextareaProps={{ placeholder }}` to change) and its attach button sits on the left. Group avatars show a single initial (no stacking). When the goal is matching a specific/bespoke design, follow [`design-matching.md`](design-matching.md) - the Classify -> Spec -> Route -> Ground -> Build -> Verify pipeline, with a v14 reskin cheat-sheet appendix and a capture-based verify gate (`tsc`/`build` passing is not visual parity).
