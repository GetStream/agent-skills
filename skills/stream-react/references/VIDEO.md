...
**This is not product selection.** The [Use Case Matching](../builder.md) table decides *what to build* (which products, which blueprints). This setting only *labels the app you already decided to build*. Same vocabulary ("livestreaming", ...), different layer: setting it neither pulls in extra blueprints nor replaces any of the Setup CLI above (a livestream app still needs the role grants regardless of this value).
