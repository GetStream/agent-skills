...
Pick the track from the user's input. Building (web + platform) and docs go to dedicated sub-skills; **CLI tasks are handled here** (see Stream CLI below) - the `getstream` CLI underlies every track.
...
>
> Install a missing peer on demand per its policy in `peers.yaml` (Glob its path, run its install command - `getstream skills <name>`), then invoke via the `Skill` tool or Read it inline. Don't call `Skill` before the Glob, and don't stop after naming the track.
...
| Operational verb + Stream noun ("list calls", "show channels", "any flagged", "find users", "check {anything}"), or a literal `stream` command (`getstream api`, `getstream init`, `getstream login`) | **Stream CLI** - handle here (below) |
