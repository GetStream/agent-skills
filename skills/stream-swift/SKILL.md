---
name: stream-swift
description: "Build, integrate, migrate to, and answer how-to questions for Stream Chat, Video, and Feeds in Swift / SwiftUI / UIKit / iOS apps - including AI assistants and agentic experiences (ChatGPT-style apps, LLM bots in chat, streaming responses, thinking indicators, client-side MCP tools) with the StreamChatAI components. Routes each request to the exact official iOS docs page, fetches it live, and applies it - with a curated setup flow, an AI integration runbook, a Sendbird -> Stream Chat migration runbook, and iOS-specific pitfalls."
...
3. **Mode** - one of:
   ...
   - **AI assistant / LLM / agent** ("build a ChatGPT-style app", "add an AI assistant / support bot / copilot", "stream the LLM response", "thinking indicator", "stop generating", "render markdown from the model", "let the agent call tools on the device", MCP) -> run [`ai-integration.md`](ai-integration.md): it classifies the shape (AI-first app vs a bot inside an existing messenger vs device-side tools), keeps the model on a backend agent (Stream's AI SDK or LangChain SDK - never an LLM key in the app), and wires the `StreamChatAI` components (`StreamingMessageView`, `AITypingIndicatorView`, `ComposerView`, `SuggestionsView`, `ClientTool`) into the pre-built `StreamChatSwiftUI` components. Runs Setup first for integrate / new app. A pure how-to about one AI component goes to **Docs lookup** ([`docs-map.md`](docs-map.md) "Chat - AI integrations").
