...
If `stream` does not resolve, ask the user to install it from https://getstream.io and wait - never fetch or run an install script yourself. Do not continue until `command -v getstream` resolves, unless the user chooses to paste API key and token manually.
