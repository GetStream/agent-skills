...
**Matching a provided design is a decompose-and-plan-first task, not a theming afterthought.** When a Chat request carries a target appearance (screenshot, Figma, "look like \<app\>"), run [`references/design-matching.md`](references/design-matching.md) **before** writing UI code: it decomposes the reference region by region (Step 1), routes every difference to theming / layout / functional against the installed package (Step 2), builds (Step 3), and verifies region-by-region against the reference (Step 4). The per-region routing map and the completion contract for any region you render yourself are [`references/custom-ui.md`](references/custom-ui.md). Implement **every** region, the composer included; a region left at the SDK default is a FAIL, not a "known cosmetic gap." Report a region as unmatched only when it is genuinely impossible (say what + why), never because it is risky or more effort.

---

## Design-matching discipline (reasoning rules)

These are cross-cutting reasoning rules for matching a reference design. They catch the *class* of mistake a single fact never does — they generalize to regions not yet enumerated. They apply on top of [`references/design-matching.md`](references/design-matching.md) (the procedure) and [`references/custom-ui.md`](references/custom-ui.md) (the per-region facts and recipes). When a rule and a region-doc disagree, this file wins (last rule below).
