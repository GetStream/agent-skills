...
**Ringing** is opt-in and SDK-managed. Install `@stream-io/react-native-callingx` and follow the manifest-selected `/incoming-calls/ringing-setup/react-native/` and `/incoming-calls/ringing-setup/expo/` pages. The SDK wires CallKit (iOS) and Android Telecom for system-level incoming-call UI; configuration runs through `StreamVideoRN.setPushConfig({ ... })` at app start. On iOS, call `StreamVideoReactNative.voipRegistration()` once from `application:didFinishLaunchingWithOptions:` (Expo injects this via the SDK config plugin); the SDK manages PushKit, so do not add any PushKit delegate code to the AppDelegate yourself. An optional per-platform flag `skipIncomingPushInForeground: true` (under `ios` / `android`) suppresses the CallKit/Telecom UI when the app is already foregrounded so the in-app ringing UI can take over.
