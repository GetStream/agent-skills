# Stream React Native - custom components, slot overrides, and the per-region contract

**Read this whenever you route a region to a mechanism, and whenever you write your OWN component
for a region a prebuilt component normally renders** - even a single region, and even when you wire
it through the documented `WithComponents overrides={{ … }}` / component-prop path. Passing a theme
key or a prop to a prebuilt component is not that; just confirm the key against the installed
package.

This file is the RN **mechanism reference + completion contract**. It carries three things:

1. **Which axis a difference routes to** (functional / theming / layout) and whether the completion
   contract applies at all - [Three axes](#three-axes-of-customization-internalize-this-first).
2. **The per-region routing map** - every Chat, Video, Feeds, and cross-cutting region, what to
   check on it, and the exact mechanism it routes to, with the deep-dives for the regions that keep
   shipping wrong (the composer, in-bubble metadata, the attachment picker, the long-press menu).
   **Each row is a contract row**: a region you render yourself ends `Reproduce it` /
   `N/A - <design reason>` / `GAP - not implemented`, never blank.
3. **The sub-feature inheritance rule** - overriding a composite slot silently drops everything the
   default drew unless you reproduce it
   ([the completion contract](#overriding-a-slot-inherits-all-of-its-sub-features)).

It does **not** carry the *procedure*. **Matching a reference design (screenshot / Figma / a
Sendbird original)?** Run [`design-matching.md`](design-matching.md) - it owns the orchestration,
the design analysis, the routing + ownership manifest, the build fan-out, and the verify loop, and
it routes back here for every region's mechanism and contract rows.

**Names are confirmed, never remembered.** Every theme key, slot, prop, and hook named below is
verified against a specific SDK version and drifts across releases. Confirm each one against the
**installed package** (`node_modules/stream-chat-react-native-core`, `@stream-io/video-react-native-sdk`,
`@stream-io/feeds-react-native-sdk`) and the manifest-selected docs page ([`DOCS.md`](DOCS.md))
before you rely on it - that confirmed name is what lands in the `Plan` column of
`design-analysis.md` ([`design-matching.md`](design-matching.md) Step 2).
...
**Does the completion contract apply? One question: are you rendering your own component for a
region the SDK normally draws?** Functional and Theming rows say **no** - you replace nothing, so
confirm the prop / theme key and move on. A **Layout / structure** row says **yes**: you inherit
every sub-feature the default drew, so that region owes
[the completion contract](#overriding-a-slot-inherits-all-of-its-sub-features) - whether it is one
narrow slot or a whole message row. Don't talk yourself out of it because "it's only one slot": a
single custom `MessageContentBottomView` is exactly the case that silently drops receipts or
grouping. **Feeds has no prebuilt RN UI at all**, so every Feeds region is custom by definition and
the contract always applies there.
...
## Region contract rows: the checklist + routing map (walk every row)
...
reaches it (per [Three axes](#three-axes-of-customization-internalize-this-first)). Don't skip a row
because it "looks standard"; verify it against the default.

**Every row here is a contract row.** In a design match, each one carries a spec (measured in
[`design-matching.md`](design-matching.md) Step 1), a `Plan` (the mechanism you commit to in Step 2),
an owner (the manifest, Step 2), and a terminal status a worker fills as it builds: **`Reproduce it`**
/ **`N/A - <genuine design reason>`** / **`GAP - not implemented`**. A blank row is incomplete, and
"deferred" / "minor" / "close enough" is not a status - see
[the completion contract](#overriding-a-slot-inherits-all-of-its-sub-features).

The **Route to** column names the *mechanism*; **confirm the exact theme key / slot / prop name** in
the manifest-selected docs ([`DOCS.md`](DOCS.md)) and the installed package, not from memory - the
confirmed name is what enters the `Plan` column. For the rules behind each axis see the
[Theming Blueprint](./CHAT-REACT-NATIVE-blueprints.md#theming-blueprint) and the
[Component Override Blueprint](./CHAT-REACT-NATIVE-blueprints.md#component-override-blueprint).

Per region, the attributes to pin down are: color, background color, border, border radius,
padding / gap, and typography (family, **weight**, font + line size). How to measure and sample them
is [`design-matching.md`](design-matching.md) Step 1; where they get recorded is that step's
`design-analysis.md`.
...
6. **Do both senders + verify:** incoming *and* outgoing; confirm the metadata sits **inside** the bubble background, does not clip the right/bottom edge, and the default outside footer is gone (not duplicated). This is a composite-slot change — [the completion contract](#overriding-a-slot-inherits-all-of-its-sub-features)'s "reproduce every sub-feature" contract applies (grouping, edited/deleted state, quoted parent still render).
...
## Overriding a slot inherits ALL of its sub-features
...
- **`N/A - <genuine design reason>`** - a real *design* reason the target doesn't need it (e.g. read
  ...
  (`replies:false`, per the thread-scope decision in [`design-matching.md`](design-matching.md) Step 2); if threads are still enabled server-side
...
**Where the filled rows go.** In an orchestrated design match, a feature worker returns its filled
contract rows in its report and the coordinator collects them; every row then needs a judge and a
row in the discrepancy table before exit ([`design-matching.md`](design-matching.md) Step 4d's
completeness gate). A contract row nobody verified is not `Reproduce it` - it is unverified.
