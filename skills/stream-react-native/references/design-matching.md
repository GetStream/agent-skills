...
#### Message metadata inside the bubble (WhatsApp / iMessage) — a worked relayout
...
1. **Render the metadata in an in-bubble slot** — `MessageContentBottomView` (below the text, inside the bubble) or `MessageContentTrailingView` (same row as the text, trailing edge). These are *inside* the bubble background; `MessageFooter`/`MessageHeader` are *outside* it.
...
4. **Reuse `MessageStatus` for the ticks** — don't hand-roll single/double-tick logic (you'll desync read vs delivered). Recolour via theming, and **sample the tick colour off the reference** rather than assuming a "known" brand hue — messenger read-ticks are frequently a different colour than you'd guess ([Follow EVERY color](#follow-every-color-from-the-reference--sample-it-dont-guess-and-sample-each-sub-part)).
5. **Reactions share this bottom row in Telegram-style designs** — if so, render them in the same in-bubble slot and set both `ReactionListTop`/`ReactionListBottom` to `() => null` (see the reactions table above); "bottom" reaction lists render *outside* the bubble.
...
**Send / mic lives INSIDE the input pill by default** (the SDK's `OutputButtons`, in `MessageInputTrailingView`), and it is **stateful**: at rest it shows mic/audio, and it **swaps to the send button when the input has text**. So the composer is at least **two screenshots** — at-rest and typing — rendered by the same slot. To keep the swap and audio-recording working, **reuse `OutputButtons` / `AudioRecordingButton`; do not hand-roll a send button.** Moving send/mic to the *right of the field* (outside the pill) means rendering them in `MessageComposerTrailingView` instead of `MessageInputTrailingView` — override the slot, don't just theme it.

**The attach (`+`) button is TWO things — verify both facets in both states.** It is (1) a **trigger** that opens/closes the picker AND (2) a **stateful icon**: `+` when closed, a **keyboard glyph when the picker is open** (the "return to the keyboard" affordance every messenger has). Two recurring misses:
- **Do not drop in the raw SDK `<AttachButton />` and assume it matches.** It renders as a `Button variant="secondary" type="outline"` — a **bordered/ringed** button with `icons.Plus`. WhatsApp's is a **borderless dark `+`**. Using it inherits the SDK look and discards the styling you matched (an *idiomatic ≠ matching* regression — [`../RULES.md`](../RULES.md)).
...
**Composer verification gate (do NOT leave the composer until all pass — the recurring defect):**
- [ ] **Both states** captured — at-rest (mic/camera) **and** typing (send). Drive typing from a temp child with `useMessageComposer().textComposer.setText('hello')` ([SIMULATOR-VERIFICATION.md](SIMULATOR-VERIFICATION.md) §4).
...
**The canonical messenger attach sheet IS Stream's `AttachmentPicker` — override the bar, don't rebuild the surface.** WhatsApp / Telegram / iMessage attach sheets are all the same shape: an **action-tile selection bar on top + a media gallery below** — which is exactly `AttachmentPicker`'s default layout. So the default move is: **override only `AttachmentPickerSelectionBar`** (via `WithComponents`) to match the tiles, and **keep the SDK gallery + the `AttachButton`/`openPicker` lifecycle + the attachment-preview + permission flow.** Do **NOT** build a standalone `Modal` with your own sheet state — that bypasses the SDK gallery, previews, and permissions, and re-implements infrastructure the SDK already has. Only replace the whole `AttachmentPicker` host (via `OverlayProvider`'s `AttachmentPickerComponent`) if the reference's selection bar must sit **below** the gallery — a real, sizeable change to call out, not the default.
...
> - **Custom header rendered *inside* `<Channel>`** (`headerShown: false` + your own header `View` above `MessageList`, the common WhatsApp/Telegram floating-header pattern): the Channel already fills the screen from `y=0`, so there is **nothing above it to offset** → both props stay **`0`/unset**. A non-zero value here is the bug, not the fix: it over-compensates the keyboard-avoiding view and mis-computes the picker snap. Set both to `0` and confirm with the picker open and the keyboard up; don't leave a dead `insets.top + HEADER_HEIGHT` value in place.
>
