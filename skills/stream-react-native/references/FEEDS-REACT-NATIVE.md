...
- **`foryou` ships with no `activity_selectors` configured and returns empty until you configure one.** This is server-side on the feed group, not a client setup step. Configure via dashboard or `stream api UpdateFeedGroup --id foryou --request '{"activity_selectors":[{"type":"popular","min_popularity":1,"cutoff_window":"7d"}]}'`. With the `popular` selector, an activity needs popularity >= `min_popularity` to appear (formula: `reactions + comments*2 + bookmarks*3 + shares*3`); seed at least one reaction per activity in demos.
