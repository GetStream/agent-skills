...
Rules: the `stream` skill's [`RULES.md`](../stream/RULES.md) (secrets, strict mode protection, package manager). **CLI:** onboard with `stream init` before any workflow that needs the `stream` CLI; see [`../stream/SKILL.md`](../stream/SKILL.md) > Stream CLI for usage.
