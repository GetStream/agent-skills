...
compatibility: Requires an Android Studio / Gradle project (Kotlin). The `stream` CLI (binary name `stream`) is the default path for the credentials flow (API key fetch, token mint, and any product-specific setup - see `credentials.md`); only optional when the user pastes the API key and token themselves.
metadata:
  author: GetStream
allowed-tools:
  - Read
